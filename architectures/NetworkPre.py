...
class FeatureNet(nn.Module):
    ...
    def forward(self, the_img, labels=None, conj_ids=None, base_ids=None, test=False,Epoch =None):
        if labels is None:
    ...
    def open_forward(self, the_input, labels, conj_ids, base_ids, test):
        ...
        test_cosine_scores, supp_protos1, fakeclass_protos1, loss_cls, loss_funit,recip_unit = self.task_proto((support_feat,query_feat,openset_feat), (supp_idx,base_ids), cls_label, test=test)
        ...
        _, supp_protos_aug, fakeclass_protos_aug, loss_cls_aug, loss_funit_aug,recip_unit = self.task_proto((supopen_feat,openset_feat,query_feat), (open_idx,base_ids), cls_label, test=test)
    ...
    def task_proto(self, features, cls_ids, cls_label,test=False,mixup_part=None):
